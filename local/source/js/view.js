...
if (window.jQuery) {
  var View = new function () {
    ...
    var win = $(window);
    var doc = $(document); // внешние переменные
    ...
    this.screenSizeIsAdaptive = null; // проверка устройства десктоп или адатив

    this.mobileAndTabletCheck = null; // проверка наличия скроллбара у окна браузера

    this.winHaveScroll = null; // ширина скроллбара у окна браузера

    this.scrollBarWidth = null; // длительность анимации эффектов переключения всех переключающихся блоков

    this.animateDuration = 200; // breakpoints
    ...
    this.getVisibleElemOnScroll = function (elemJQ, reserveHeight) {
    ...
    this.allClassPage = []; // для защиты от повторного скачивания апи карт

    this.loadYandexApi = false; // инициализация всех яндекс карт на сайте
      ...
      function initMaps() {
        ...
        $('#ymaps-multi-city').each(function () {
          ...
          if (!mapItem.hasClass('init-map')) {
            ...
            var loadMapData = function loadMapData(dataJsonPath) {
              // путь к JSON файлу с данными для карты
              var pathJsonMultiCustom = dataJsonPath || mapItem.data('json-path'); // удалить все метки на карте

              objectManager.removeAll();
              mapObject.geoObjects.removeAll(); // Загружаем GeoJSON файл с описанием объектов.

              $.getJSON(pathJsonMultiCustom).done(function (response) {
                // Добавляем описание объектов в формате JSON в менеджер объектов.
                objectManager.add(response); // Добавляем объекты на карту.

                mapObject.geoObjects.add(objectManager); // устанавливаем нужный центр и зум карты, чтобы было видно все метки и задаем нужный отступ меток от края карты
            ...
            objectManager.objects.options.set('iconImageOffset', [-24, -60]); // контент для метки кластера

            var MyIconContentLayout = ymaps.templateLayoutFactory.createClass('<div style="color: #ffffff; font-weight: 500; font-size: 17px;">{{ properties.geoObjects.length }}</div>'); // стилизация метки кластера
            ...
            loadMapData(); // проброс функции обновления данных карты наружу

            mapItem.data('loadMapData', loadMapData); // переключение города и его меток на карте
        ...
        $('.ymaps-with-input').each(function () {
          ...
          if (!mapItem.hasClass('init-map')) {
            ...
            var setPositionMap = function setPositionMap() {
              mapObject.setBounds(mapObject.geoObjects.getBounds(), {
                checkZoomRange: true,
                zoomMargin: 50
              }).then(function () {
            ...
            var geocode = function geocode() {
              ...
              var request = locationInput.val(); // Геокодируем введённые данные.

              ymaps.geocode(request).then(function (res) {
                var obj = res.geoObjects.get(0),
                    error; // Об оценке точности ответа геокодера
              ...
              function showResult(obj) {
                ...
                var bounds = obj.properties.get('boundedBy'); // Рассчитываем видимую область для текущего положения пользователя.

                var mapState = ymaps.util.bounds.getCenterAndZoom(bounds, [mapItem.width(), mapItem.height()]); // Убираем контролы с карты.

                mapState.controls = []; // Передвигаем карту по найденным координатам
            ...
            objectManager.objects.events.add(['balloonopen', 'redraw'], function (e, a) {
              if (win.width() < 820) {
                $('.fancybox-slide').scrollTop(400); // fullscreenControl.enterFullscreen();
            ...
            loadMapData(); // Слушаем клик на карте.
            ...
            var locationInput = $('#location-input'); // сообщение об ошибке

            var notice = $('#notice'); // Подключаем поисковые подсказки к полю ввода.

            var suggestView = new ymaps.SuggestView('location-input'); // При клике по кнопке запускаем верификацию введёных данных.
            ...
            mapObject.controls.add(fullscreenControl); // закрытие баллуна по кнопке внутри него

            doc.off('click.closeBallon').on('click.closeBallon', '.js-select-pharmacy', function (e) {
              mapObject.balloon.close(); // переведем карту из «полноэкранный режим»
    ...
    this.youtubeVideoPlayed = null; // трансформация строки в camelCase стиль
    ...
    this.init = {
      ...
      global: {
        ...
        toggleDropdownHover: function toggleDropdownHover() {
          ...
          allParents.each(function (ind) {
            ...
            var timeout = null; // инициализации и закрытие при загрузке страницы

            content.click(function (event) {
              event.stopPropagation();
            });
            parent.addClass('init-dropdown'); // переключение метода открытия для адаптива и десктопа

            function toggleDropdownHover() {
              if (View.mobileAndTabletCheck) {
                // открытие по клику
                togglerClick(); // снятие ненужных обработчиков
                ...
                parent.off('mouseover.toggleDropdownHover').on('mouseover.toggleDropdownHover', function (event) {
                  ...
                  event.stopPropagation(); // переключение состояния
        ...
        ajaxLoadAppendBody: function ajaxLoadAppendBody() {
          ...
          $('.js-ajax-load-toggler').each(function () {
            ...
            toggler.off('click.ajaxLoad').on('click.ajaxLoad', function (event) {
              ...
              $.get(url).done(function (response) {
                ...
                preloader.remove(); // реинициализация скриптов после аякса
        ...
        adaptiveHeightHoverMenu: function adaptiveHeightHoverMenu() {
          ...
          function delayHoverMenu() {
            allParents.each(function () {
              ...
              if (!View.mobileAndTabletCheck) {
                ...
                items.each(function () {
                  var curItem = $(this); // обработка прихода курсора на пункта меню
                  ...
                  curItem.off('mouseover.showInnerMenu').on('mouseover.showInnerMenu', function () {
                    ...
                    setTimeout(function () {
                      ...
                      if (curItem.hasClass('hover')) {
                        curItem.addClass('active'); // установка высоты навигации в зависимости от высоты выпадющего меню
                  ...
                  curItem.off('mouseout.hideInnerMenu').on('mouseout.hideInnerMenu', function () {
                    var nPosXStart = nCurPosX; // curItem.removeClass('hover');
        ...
        toggleAccordionOnlyAdaptive: function toggleAccordionOnlyAdaptive() {
          var timeout = null; // accordion только для адаптива
            ...
            allParents.each(function () {
              ...
              if (win.width() <= maxWidth) {
                ...
                toggler.off('click.toggleAccordion').on('click.toggleAccordion', function (event) {
                  event.stopPropagation();
                  event.preventDefault(); // View.control.closeAll(['closeAllAccordionAdaptive'])
        ...
        toggleClassByHover: function toggleClassByHover() {
          var allItems = $('.js-class-hover');
          var parent = $('.js-class-hover-parent');
          var timeout = null;
          allItems.each(function () {
            var item = $(this);
            var itemClass = item.data('class'); // обработка прихода курсора на элемента
        ...
        mobileMenu: function mobileMenu() {
          ...
          $hamburgerMenuAll.each(function () {
            ...
            $hamburgerMenuToggles.off('click.hamburger').on('click.hamburger', function (e) {
              ...
              $self.addClass('open');
              $self.next('.js-hamburger-menu__sub-list').find('.js-hamburger-menu__btn-back').off('click.hamburger').on('click.hamburger', function () {
        ...
        initCarousel: function initCarousel() {
          if (window.Swiper) {
            ...
            var initSwiper = function initSwiper(elemJQ, optionSwiper) {
            ...
            var toggleClassSwiper = function toggleClassSwiper(parent, widthAllItems) {
              parent.toggleClass('swiper-empty', 'parent.width() > widthAllItems');
            }; // ******************************************************************************


            // одиночная карусель
            var initSingleSwiper = function initSingleSwiper() {
              initSwiper($('.js-carousel--main-banner'), optionSwiper);
            };

            // одиночная карусель
            var initSingleSwiperArticles = function initSingleSwiperArticles() {
        ...
        fixedBlockWhenScrolling: function fixedBlockWhenScrolling() {
          $('.js-fixed--up').each(function (index) {
            ...
            function fixedBlockWhenScrolling() {
              var docScroll = doc.scrollTop(); // фиксация блока
              ...
              if (!block.hasClass('active-fixed') && blockStart <= docScroll) {
                ...
                block.removeClass('active-fixed'); // block.removeClass('active-fixed').css('padding-top', 0)
              }
            }

            block.css('padding-top', blockHeight).addClass('init-fixed');
            fixedBlockWhenScrolling(); // инициализаци с индексом на случай множественных блоков для фиксации

            win.off('resize.fixedBlockWhenScrolling' + index).on('resize.fixedBlockWhenScrolling' + index, function () {
              blockHeight = content.outerHeight();
              blockStart = block.offset().top;
              block.css('padding-top', blockHeight);
              if (requestAnimationFrame) requestAnimationFrame(fixedBlockWhenScrolling);else fixedBlockWhenScrolling();
            });
        ...
        collapsibleMenu: function collapsibleMenu() {
          $('.js-collapsible').each(function () {
            var parent = $(this);
            var items = parent.find('.js-collapsible__item');
            var quantity = parent.data('collapsible'); // если елементов больше разрешенных и родитель не имеет класса инициализации свернуть лишние элементы в аккордион

            if (items.length > quantity && !parent.hasClass('init-collapsible')) {
              parent.addClass('init-collapsible'); // добавление разметки для аккордионов
              ...
              var accordionContent = $('<div class="js-accordion__content"></div>'); // пробегаем по всем элементам перенося лишние в аккордион
        ...
        setRangeInputs: function setRangeInputs() {
        ...
        scrollToBlock: function scrollToBlock() {
          $('.js-scroll-to').each(function () {
            var _this = $(this);

            _this.off('click.scrollToBlock').on('click.scrollToBlock', function (event) {
              event.preventDefault(); // величина недоезда блока до верха в атрибуте ссылки

              var offset = $(_this).data('offset') || 0; // ID блока до, которого скролить
            ...
            if (popups.length) {
              popups.fancybox({
                ...
                beforeShow: function beforeShow() {
                  // компенсация скролла
                  body.css({
                    paddingRight: View.scrollBarWidth + 'px'
                  });
                },
        ...
        checkAnyCheckbox: function checkAnyCheckbox() {
          $('.js-check-any').each(function () {
            ...
            checkboxes.off('change.checkAnyCheckbox').on('change.checkAnyCheckbox', function () {
              if (Array.from(checkboxes).some(function (elem) {
        ...
        tooltipPosition: function tooltipPosition() {
          ...
          var timeout = null; // вычисление положения

          View.init.global.tooltipPosition.calcPosition = function () {
            tooltips = $('.js-tooltip-position');
            tooltips.css('marginLeft', ''); // ширина паддинга контейнера страницы
            ...
            for (i = 0; i < tooltips.length; i++) {
              ...
              delta = tooltips[i].getBoundingClientRect().left + tooltips[i].offsetWidth - Math.min(window.innerWidth - View.scrollBarWidth - containerPadding, (window.innerWidth - View.scrollBarWidth) / 2 + 720);
        ...
        triggerClick: function triggerClick() {
          ...
          allSenders.each(function () {
            var sender = $(this); // массив из ID отправителя для связывания триггеров со своими приемщиками

            var triggerIdArr = sender.data('triggerId') ? String(sender.data('triggerId')).split(',') : false; // обработка клика отправителя триггера
            ...
            sender.off('click.triggerClick').on('click.triggerClick', function (event) {
              ...
              if (triggerIdArr) {
                ...
                allRecipients.each(function () {
                  var recipient = $(this); // инициализация клика при совпадении ID отправителя и приемщика
        ...
        // анимация чисел
        animateDigits: function animateDigits() {
          if ($.fn.spincrement) {
            var animateDigits = function animateDigits() {
              $('.js-animate-digits').each(function () {
                ...
                if (_this.offset().top < win.height() + win.scrollTop() - 50 && !_this.hasClass('animated')) {
                  ...
                  _this.addClass('animated'); // фиксация ширины во время анимации
        ...
        initMapsDelayScroll: function initMapsDelayScroll() {
          $('.js-map').each(function () {
            var map = $(this);
            var timeout = null;

            function initMaps() {
              console.log('initMapsDelayScroll', View.loadYandexApi); // инициализируем если карта еще не инициализировалась
            ...
            win.off('scroll.initMapsScroll').on('scroll.initMapsScroll', function () {
              // clearTimeout(timeout)
              // timeout = setTimeout(function () {
              if (View.getVisibleElemOnScroll(map, 500)) initMaps(); // }, 10000)
            });
        ...
        closeAll: function closeAll() {
          ...
          if (View.mobileAndTabletCheck) {
            $('.js-close-all-hover').off('click.closeAll').on('click.closeAll', function (e) {
              View.control.closeAll(); // View.init.local.toggleClassBlock.closeAllClassBlocks()
      ...
      local: {
        ...
        lazyLoad: function lazyLoad(scope) {
          if ($.fn.Lazy) {
            View.init.local.lazyLoad.instance = $('.js-lazy[data-src]', scope).Lazy({
              ...
              afterLoad: function afterLoad(element) {
                element.addClass('lazy-loaded'); // очистка фона после загрузки изображения
        ...
        filterItems: function filterItems() {
          $('.js-filter-items').each(function () {
            ...
            var items = parent.find('.js-filter-items__item'); // инициализация предустаноленного индекса

            filterItems(parent.attr('data-index'));
            parent.addClass('init-filter-items'); // обработка переключения
        ...
        toggleClassBlock: function toggleClassBlock(scope) {
          var allParents = $('.js-class', scope);
          var allClass = [];
          allParents.each(function () {
            var parent = $(this); // кастомный класс для переключения либо дефолтный
            ...
            parent.find('.js-class__toggler').off('click.toggleClass').on('click.toggleClass', function (event) {
              ...
              if (!parent.hasClass(activeClass)) {
                ...
                parent.addClass(activeClass); // обновление lazyload картинок внутри таба

                View.init.local.lazyLoad(); // показ первого сабменю при открытиии каталога
        ...
        toggleClassPage: function toggleClassPage(scope) {
        ...
        showSearch: function showSearch(scope) {
                ...
                if (input.val().length >= 3) {
                  $.ajax({
                    ...
                    'success': function success(data) {
                      ...
                      content.html($('.js-search__content', data).html()).slideDown(300);
        ...
        // accordion group
        toggleAccordionGroup: function toggleAccordionGroup(scope) {
          $('.js-accordion--group', scope).each(function () {
            ...
            allAccordions.each(function () {
              var accordion = $(this);
              var content = accordion.find('.js-accordion__content'); // инициализация и закрытие всех аккордионов кроме тех что с классом open-accordion

              if (!accordion.hasClass('open-accordion')) content.slideUp(0);
              accordion.addClass('init-accordion'); // переключение состояния аккордиона по клику

              accordion.find('.js-accordion__toggler').off('click.toggleAccordion').on('click.toggleAccordion', function (event) {
                event.stopPropagation();
                event.preventDefault();
                closeOtherAccordions(); // переключение состояния аккордиона
        ...
        toggleSelect: function toggleSelect(scope) {
          ...
          allParents.each(function () {
            ...
            var input = parent.find('.js-select__value'); // инициализации и закрытие селекта при загрузке страницы

            content.fadeOut(0);
            parent.addClass('init-select'); // инициализация предустановленного значения селекта
            ...
            toggler.off('click.toggleSelect').on('click.toggleSelect', function (event) {
              event.stopPropagation();
              event.preventDefault(); // селект заблокирован при наличии класса
        ...
        clippedText: function clippedText(scope) {
          $('.js-clipped', scope).each(function (ind) {
            var parent = $(this);
            var content = parent.find('.js-clipped__content');
            var toggler = parent.find('.js-clipped__toggler');
            var timeout = null;
            var textInit = content.html(); // высота текста из атрибута или дефолтная, которую стоит сворачивать

            var clippedHeight = parent.data('clippedHeight') || 100; // колличество символов для показа сокращенной версии

            var clippedSymbols = parent.data('clippedSymbols');
            var winWidth = null;

            function clippedText() {
              winWidth = win.width(); // сбросс высоты контента
              ...
              var initialHeightContent = content.outerHeight(); // инициализация плагина, если длинна текста больше допустимой

              if (initialHeightContent > clippedHeight) {
                parent.addClass('init-clipped');
                close(); // переключение сворачивания контента
                ...
                toggler.off('click.clippedText').on('click.clippedText', function () {
                  if (parent.hasClass('open-clipped')) {
                    ...
                    content.css({
                      maxHeight: initialHeightContent
                    });
                  }
                });
              } else {
        ...
        toggleAjax: function toggleAjax(scope) {
        ...
        validateForm: function validateForm(scope) {
          if ($.fn.parsley) {
            $('[data-parsley-validate]', scope).parsley(); // обработка валидации поля с ощибкой
        ...
        initTooltip: function initTooltip(scope) {
          if ($.fn.tooltipster) {
            ...
            $('.js-tooltip', scope).each(function () {
              ...
              var options = {
                ...
                functionInit: function functionInit(instance, helper) {
                  ...
                  var dataOptions = toggler.data('tooltipster'); // примениение параметров в атрибуте при их наличии
        ...
        // загрузка файла
        inputFile: function inputFile(scope) {
          $('.js-input-file', scope).each(function () {
            var parent = $(this);
            var name = parent.find('.js-input-file__name'); // // отслеживание события выбора файла
    ...
    this.control = {
      functionsArr: {
        ...
        closeAllMobileMenu: function closeAllMobileMenu() {
          View.init.global.mobileMenu.closeAll();
        },
        closeAllClassBlocks: function closeAllClassBlocks() {
          View.init.local.toggleClassBlock.closeAllClassBlocks();
        },
        closeAllDropdownHover: function closeAllDropdownHover() {
        ...
        closeAllClassPage: function closeAllClassPage() {
          View.init.local.toggleClassPage.closeAllClassPage();
      ...
      closeAll: function closeAll(exclude) {
        exclude = exclude || []; // перебор массива со всеми имеющимися функциями для управлени
        ...
        for (var fn in View.control.functionsArr) {
          ...
          } else {// console.log('View.control.functionsArr[fn]', fn)
          }
        }
  ...
  $(function ($) {
    var body = $(document.body); // body.removeClass('loading-page');
